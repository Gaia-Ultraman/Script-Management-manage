...
export default class App extends React.Component {
  ...
  componentDidMount() {
    ...
    this.timer = setInterval(() => {
      ...
      if (this.results.length) {
        this.results.forEach((result) => {
          ...
          if (result.from && result.from.group == 'server') {
            if (result.data && result.data.cmd == 'getAllOnline') {
              result.data.retMsg = result.data.retMsg.sort(function (a, b) {
                return sorter(a.name, b.name);
              });
              this.setState({
                showDevices: result.data.retMsg,
                allDevices: result.data.retMsg,
                currentGroup: '全部',
                checked: false,
              });
            ...
            // this.setState({
            //     //排序
            //     allDevices: allDevices.sort(function (a, b) {
            //         return sorter(a.name, b.name)
            //     }),
            //     showDevices: showDevices.sort(function (a, b) {
            //         return sorter(a.name, b.name)
            //     })
            // })
          }
        ...
        //上线的消息集合
        let onLineResults = this.results.filter(
          (result) => result.data && result.data.cmd == 'online',
        );
        //下线的消息集合
        let offLineResult = this.results.filter(
          (result) => result.data && result.data.cmd == 'offline',
        );
        if (onLineResults.length) {
          onLineResults.length > 1
            ? message.success(`${onLineResults.length}台设备 上线`)
            : message.success(`${onLineResults[0].data.retMsg.name} 上线`);
        }
        if (offLineResult.length) {
          offLineResult.length > 1
            ? message.error(`${offLineResult.length}台设备 下线`)
            : message.error(`${offLineResult[0].data.retMsg.name} 下线`);
        }
      }
      this.results = [];
    }, 3000);
  ...
  handleGroup = (group, removeCheck) => {
    ...
    if (group.type == 1) {
      ...
      let addDevices = cloneAllDevices.filter((v) => {
        ...
        return Object.keys(group.addRegs).reduce((pre, cur, index) => {
          return (
            pre &&
            group.addRegs[cur].reduce((p, c, i) => {
              if (!c) return p;
      ...
      if (!empty) {
        ...
        let deleteDevices = cloneAllDevices.filter((v) => {
          ...
          return Object.keys(group.deleteRegs).reduce((pre, cur, index) => {
            return (
              pre &&
              group.deleteRegs[cur].reduce((p, c, i) => {
                if (!c) return p;
                let reg = new RegExp(c);
    ...
    newShowDevices = cloneAllDevices.filter((v) => {
      return group.data.indexOf(v.id) != -1;
    });
  ...
  handleSlider = (value) => {
    const { showDevices } = this.state;
    this.sendMessage(
      { codeType: 'device', cmd: 'setScreenBrightness', brightness: value },
      { group: 'phone', id: showDevices.filter((v) => v.checked).map((v) => v.id) },
    );
  };
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <div>
            ...
            <Checkbox style={{ marginLeft: '96px' }} checked={checked} onClick={this.handleCheck}>
              全选
            </Checkbox>
            <span style={{ paddingLeft: '30px' }}>
              当前分组数量:
              <span style={{ color: 'green', fontSize: 20 }}>{showDevices.length}</span>
            </span>
            <span style={{ paddingLeft: '30px' }}>
              已经勾选数量:
              <span style={{ color: 'green', fontSize: 20 }}>
                {showDevices.filter((v) => v.checked).length}
              </span>
            </span>
          </div>
        ...
        <div className={styles.content}>
          ...
          <Groups
            ...
            handleBack={(group) => this.handleGroup(group, true)}
            devices={showDevices}
          />
          {/* 手机列表 */}
          <Cards
            ...
            onChecked={this.handleCardCheck}
            sendFunc={this.sendMessage}
            handleDevice={this.groupsRef && this.groupsRef.handleDevice}
          />
        </div>
        {/* 命令控制面板 */}
